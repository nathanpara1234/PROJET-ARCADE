...
# Bat  à la case (7, 5)
# Player à la case (7, 1) assez loin pour ne pas déclencher de collision immédiate
MAP_TEXT ="""
    width: 15
    height: 10
    ---
...
def test_bat_moves_at_constant_speed() -> None:
    ...
    # Le bat démarre loin des bords → il se déplace forcément (pas de demi-tour)
    assert dx != 0 or dy != 0
    distance = sqrt(dx**2 + dy**2)
    assert abs(distance - BAT_MOUVEMENT_SPEED) < 0.01


# ── Tests d'intégration (avec fenêtre) ────────────────────────────────────────
...
def test_player_touching_bat_triggers_restart(window: arcade.Window) -> None:
    ...
    bat = view.enemies[0]
    # Téléporter le joueur sur la chauve-souris
...
def test_bat_killed_by_sword(window: arcade.Window) -> None:
    ...
    # Positionner l'épée sur la chauve-souris et l'activer
...
def test_blob_killed_by_sword(window: arcade.Window) -> None:
    """Le blob est retiré du jeu quand l'épée le touche."""
    game_map = load_map_from_string(MAP_TEXT_BLOB)
    view = GameView(game_map)
    window.show_view(view)
