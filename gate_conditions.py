...
def condition_is_true(condition: GateCondition, switch_states: dict[str, bool]) -> bool:
    # Calcule si une condition de portail est vraie.
    # La condition vient du YAML, par exemple:
    ...
    if key == "and":
        if not isinstance(value, list):
            return False
        return (
            condition_is_true(value[0], switch_states)
            and condition_is_true(value[1], switch_states)
        )
    ...
    if key == "or":
        ...
        return (
            condition_is_true(value[0], switch_states)
            or condition_is_true(value[1], switch_states)
        )
